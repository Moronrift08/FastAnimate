...
class NeuralRender(Renderer):
    ...
    def render_mesh(
        ...
        # images = normal.permute(0, 3, 1, 2).unflatten(0, (1, 6)).detach().cpu().numpy()*255
        # images_input = images


        # images_input = images.transpose(0, 3, 1, 4, 2).reshape(-1, images_input.shape[1] * images_input.shape[3], 3) # [B*output_size, V*output_size, 3]
