...
class Camera(nn.Module):
    def __init__(self, C2W, fxfycxcy, h, w):
        ...
        def getProjectionMatrix(znear, zfar, fovX, fovY, shiftX, shiftY):
            ...
            tan_half_fov = np.tan(0.5 * np.deg2rad(49.1))
            proj_matrix = torch.zeros(4, 4, dtype=torch.float32, device=fovX.device)
        ...
        self.full_proj_transform = self.world_view_transform.unsqueeze(0) @ self.projection_matrix.unsqueeze(0) # [V, 4, 4]
