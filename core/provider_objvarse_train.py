...
def fetchPly_rotate_mesh(positions, data2):
    # angel_x = 90 
    radian = data2[0] # angel_x * np.pi / 180 
    Rotation_Matrix_1 = [  
        [1, 0, 0],
    ...
    # angel_y = -90 
    radian = -1*(data2[1]+np.pi/2) # angel_y * np.pi / 180
    Rotation_Matrix_2 = [  
    ...
    p = np.dot(Rotation_Matrix_2, positions.T)
    p = np.dot(Rotation_Matrix_1, p)
    positions = p.T
...
class ObjaverseDataset(Dataset):
    ...
    def __init__(self, opt: Options, training=True):
        ...
        for root, dirs, files in os.walk(Origin_path):
            for dir_name in dirs:
                ...
                self.instance_list[dir_name] = []
                for root_, dirs_, files_ in os.walk(dir_path+'/train'):
                    for dir_name_ in dirs_:
                        dir_path_ = os.path.join(root_, dir_name_)
                        dir_path_list.append(dir_path_)
                    break
        dir_path_list = sorted(dir_path_list)
        # obj_files = []
        for dir_path_ in dir_path_list:
            for root, dirs, files in os.walk(dir_path_):
                for file in files:
                    if file.endswith('.obj'):
                        self.instance_list[root.split('/')[-4]].append(os.path.join(root, file))



        # only for checking the data loading process
        for key in self.instance_list: 
            self.instance_list[key] = sorted(self.instance_list[key])
        self.items = self.instance_list['00021']
        if self.training:
    ...
    def __getitem__(self, idx):
        # 20 persons are presented in X-humans Dataset
        random_number = random.randint(0, 19)
        keys = list(self.instance_list.keys())
        selected_key = keys[random_number]
        selected_value = self.instance_list[selected_key]
        two_elements = random.sample(selected_value, 2)

        scan_path = two_elements[0]
        gt_path = two_elements[1]
        origin_smpl_ply = two_elements[0].replace('meshes_obj','SMPLX').replace('.obj','_smplx.ply')
        origin_smpl_pkl = two_elements[0].replace('meshes_obj','SMPLX').replace('.obj','_smplx.pkl')

        target_smpl_ply = two_elements[1].replace('meshes_obj','SMPLX').replace('.obj','_smplx.ply')
        target_smpl_pkl = two_elements[1].replace('meshes_obj','SMPLX').replace('.obj','_smplx.pkl')


        def get_first_sorted_file(directory, extension):
            files = [f for f in os.listdir(directory) if f.endswith(extension)]
            sorted_files = sorted(files)
            return sorted_files[0] if sorted_files else None


        obj_directory = os.path.dirname(scan_path)
        ply_directory = os.path.dirname(origin_smpl_ply)
        pkl_directory = os.path.dirname(origin_smpl_pkl)

        first_obj_file = get_first_sorted_file(obj_directory, '.obj')
        first_ply_file = get_first_sorted_file(ply_directory, '_smplx.ply')
        first_pkl_file = get_first_sorted_file(pkl_directory, '_smplx.pkl')

        A_gt = os.path.join(obj_directory, first_obj_file) if first_obj_file else None
        A_smpl_ply = os.path.join(ply_directory, first_ply_file) if first_ply_file else None
        A_smpl_pkl = os.path.join(pkl_directory, first_pkl_file) if first_pkl_file else None

        # path_to_files
        scan_objfile_path = scan_path
        scan_pngfile_path = scan_path.replace("mesh-","atlas-").replace(".obj",".jpg")
        
        ori_smpl_ply_path = origin_smpl_ply
        ori_smpl_pkl_path = origin_smpl_pkl

        A_smpl_ply_path = A_smpl_ply
        A_smpl_pkl_path = A_smpl_pkl
        A_gt_path = A_gt
        A_gt_pngfile_path = A_gt.replace("mesh-","atlas-").replace(".obj",".jpg")

        target_smpl_ply_path = target_smpl_ply
        target_smpl_pkl_path = target_smpl_pkl

        gt_objfile_path = gt_path
        gt_pngfile_path = gt_path.replace("mesh-","atlas-").replace(".obj",".jpg")

        origin_tex = torch.from_numpy(cv2.cvtColor(cv2.imread(scan_pngfile_path), cv2.COLOR_BGR2RGB).astype(np.float32)/255.0)
        origin_smpl = trimesh.load(ori_smpl_ply_path, force='mesh')

        A_gt_scan = trimesh.load(A_gt_path)
        A_gt_tex = torch.from_numpy(cv2.cvtColor(cv2.imread(A_gt_pngfile_path), cv2.COLOR_BGR2RGB).astype(np.float32)/255.0)
        A_origin_smpl = trimesh.load(A_smpl_ply_path, force='mesh')

        # Load the target pose scans and texture
        Posed_gt_scan = trimesh.load(gt_objfile_path)
        posed_gt_tex = torch.from_numpy(cv2.cvtColor(cv2.imread(gt_pngfile_path), cv2.COLOR_BGR2RGB).astype(np.float32)/255.0)

        # Load the target pose SMPL
        target_smpl = trimesh.load(target_smpl_ply_path, force='mesh')

        # Load the Apose and target pose SMPL pkl for animation between Stage1 and Stage2
        fitted_path = A_smpl_pkl
        ...
        fitted_path = target_smpl_pkl_path
