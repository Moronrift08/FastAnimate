...
from core.render.deferred_bp import deferred_bp
from core.render.gaussian_utils import render, GaussianModel
...
class GaussianRenderer:
    ...
    def render(self, gaussians, cam_view, cam_view_proj, cam_pos, c2ws=None, bg_color=None, scale_modifier=1, outputsize=2048):
        
        xyz = gaussians[:, :, 0:3].float()
        opacity = gaussians[:, :, 3:4].float()
        scaling = gaussians[:, :, 4:7].float()
        rotation = gaussians[:, :, 7:11].float()
        features = gaussians[:, :, 11:].float() # [N, 3]

        output_fxfycxcy = torch.tensor([[[1.0946174630511205, 1.0946174630511205, 0.5, 0.5]]]).to(gaussians.device).repeat(c2ws.shape[0],c2ws.shape[1],1)
        output_c2ws = c2ws.float()
        renderings, depths, alphas = deferred_bp(xyz, features, scaling, rotation, 
                                              opacity, outputsize, outputsize, C2W=output_c2ws, fxfycxcy=output_fxfycxcy, patch_size=outputsize, gaussian_model=self.gaussian_model)
        return {
            "image": renderings, # [B, V, 3, H, W]
            "alpha": alphas, # [B, V, 1, H, W]
            "depth": depths, # [B, V, 1, H, W]
        }
    def prune(self, gaussians, compatible=True):
        # gaussians: [B, N, 14]
        # compatible: save pre-activated gaussians as in the original paper
        # NOTE: Todo: Batch process
        means3D = gaussians[0, :, 0:3].contiguous().float()
        opacity = gaussians[0, :, 3:4].contiguous().float()
        scales = gaussians[0, :, 4:7].contiguous().float()
        rotations = gaussians[0, :, 7:11].contiguous().float()
        shs = gaussians[0, :, 11:].contiguous().float() # [N, 1, 3]
        ...
        gaussians = torch.cat([means3D, opacity, scales, rotations, shs], dim = 1).unsqueeze(0)
        return gaussians
    ...
    def load_ply(self, path, normalize = False, compatible=True):
        ...
        if normalize == True:
            gaussians[..., 0:3],_,_ = new_verts(gaussians[..., 0:3],0)
