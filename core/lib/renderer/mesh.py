...
sys.path.append("/hpc2hdd/home/gzhang292/project4/ICON")
...
def load_fit_body(fitted_path, scale, smpl_type='smplx', smpl_gender='neutral', noise_dict=None):
    ...
    smpl_out_tpose = smpl_model(**model_forward_params)
    ...
    model_forward_params_re = dict(
        betas=param['betas'],
        global_orient=param['global_orient'],#+(torch.rand_like(param['global_orient'])-0.5),
        body_pose=param['body_pose']+((torch.rand_like(param['body_pose'])-0.5)),
        left_hand_pose=param['left_hand_pose'],#+((torch.rand_like(param['left_hand_pose'])-0.5)),
        right_hand_pose=param['right_hand_pose'],#+((torch.rand_like(param['right_hand_pose'])-0.5)),
        jaw_pose=param['jaw_pose'],
        leye_pose=param['leye_pose'],
        reye_pose=param['reye_pose'],
        expression=param['expression'],
    ...
    smpl_out_repose = smpl_model(**model_forward_params_re)





    return smpl_out_tpose, smpl_out_repose, param['scale'], param['translation']
...
if __name__ == "__main__":
    ...
    fit_file = f'/hpc2hdd/home/gzhang292/project3/SiTH-main/data/THuman/THuman2.0_smplx/0000/smplx_param.pkl'

    econ_path = '/hpc2hdd/home/gzhang292/project3/SiTH-main/data/THuman/THuman2.0/0000/0000.obj'
    econ_obj = trimesh.load(econ_path)
    smpl_out_tpose, smpl_out_repose, scale, translation = load_fit_body(fit_file, 180.0, smpl_type='smplx', smpl_gender='male')
    econ_obj.vertices = (econ_obj.vertices - translation.numpy())/scale.numpy()
    ...
    rot_mat_t = smpl_out_tpose.vertex_transformation.detach()[0][idx[:, 0]]
    ...
    # da-pose for ECON
    rot_mat_da = smpl_out_repose.vertex_transformation.detach()[0][idx[:, 0]]
    ...
    # econ_cano.visual.material = econ_obj.visual.material
    econ_obj.vertices = econ_da_verts.numpy()
    econ_obj.export("/hpc2hdd/home/gzhang292/project4/test2/econ_re.obj")
    print("")
