...
class SMPLX(SMPLH):
    ...
    def forward(
        ...
        **kwargs
        ...
        if self.use_pca:
            left_hand_pose = left_hand_pose.unsqueeze(0)
            right_hand_pose = right_hand_pose.unsqueeze(0)

        global_orient = global_orient.unsqueeze(0)
        body_pose = body_pose.unsqueeze(0)
        jaw_pose = jaw_pose.unsqueeze(0)
        leye_pose = leye_pose.unsqueeze(0)
        reye_pose = reye_pose.unsqueeze(0)
        ...
        if (return_joint_transformation or return_vertex_transformation) and manual_scale != None and manual_transl != None:
            manual_transl = torch.tensor(list(manual_transl)).to(self.v_template.device)
            manual_scale = torch.tensor(list(manual_scale)).to(self.v_template.device)
            self.v_template_scale = ((self.v_template + manual_transl) * manual_scale).float()
            # T
            vertices, joints, joint_transformation, vertex_transformation = lbs(
                shape_components,
                full_pose,
                self.v_template_scale,
                shapedirs,
                self.posedirs,
                self.J_regressor,
                self.parents,
        ...
        elif return_joint_transformation or return_vertex_transformation :
            vertices, joints, joint_transformation, vertex_transformation = lbs(
                shape_components,
                full_pose,
                self.v_template,
        ...
        ##print("lmk_faces_idx.shape: ", lmk_faces_idx.shape)
        ##print("self.batch_size: ", self.batch_size)
        lmk_bary_coords = self.lmk_bary_coords.unsqueeze(dim=0).repeat(self.batch_size, 10, 1)    #有改动
