...
class LGM_1(nn.Module):
    ...
    def sparsify(self, t_vertices):
        ...
        # other_indices = torch.from_numpy(random_indices[10000:])
...
class LGM_2(nn.Module):
    ...
    ### 渲染准备，这里需要修改，Input_scan和GT_scan 都需要进行渲染
    ...
    def prepare_render_images(self, batch):
        ...
        for idx in range(batch_size):
            ...
            ### 第二部分：GT里scan的渲染    这里是Apose的GT，其实用不上
        ...
        input_view_num = 4 #batch['input_c2ws'].shape[1]  # 目前是1
        batch['origin_images'] = origin_images[:,:input_view_num].detach()  # 一个视角
    ...
    def for_render_gs(self, data):
        ...
        data['demo_input'] = torch.stack(results['images_input'])
    ...
    def gaussian_animation(self, Apose_gaussians, Apose_param, target_param, data=None):
        ...
        for item in range(bsz):
            ...
            model_forward_params_A = dict(
                ...
                return_joint_transformation=False,
                return_vertex_transformation=False)
            # print("smpl_model left_hand_components shape:", self.smpl_model.left_hand_components.shape)
            smpl_out_tpose = self.smpl_model(**model_forward_params_A)
    ...
    def forward(self, data, stage_1_model, step_ratio=1, lpips_weight_additon=0, training=False):
        ...
        # 获取输入数据的设备
        # 假设 to_stage1_data 是一个字典，包含多个张量
        for key, value in to_stage1_data.items():
            if isinstance(value, torch.Tensor):
                # 获取数据所在的设备
                device = value.device
                break  # 找到第一个张量的设备后就可以退出循环
        ...
        stage_1_results = self.gs.render(out_1['A_gaussians'], data['cam_view'], data['cam_view_proj'], data['cam_pos'], data['c2ws'], bg_color=bg_color, outputsize=self.opt.output_size)
        # Animate the Gaussian Avatar with LBS
        # Apose_param = data['param_apose']
        # target_param = data['param_targetpose']

        # tpose_gaussians, animated_gaussians = self.gaussian_animation(gaussians_stage1, Apose_param=Apose_param, target_param=target_param, data = data)

        

        # results['coarse_gaussians'] = animated_gaussians
        ...
        results['gaussians'] = gaussians_stage1
        ...
        results_demo = self.gs.render(final_gaussians, data['cam_view'], data['cam_view_proj'], data['cam_pos'], data['c2ws'], bg_color=bg_color, outputsize=1024)
        pred_images = results_demo['image'] # [B, V, C, output_size, output_size]
        ...
        results['A_refined'] = stage_1_results['image']
        ...
        results['final_gaussians'] = final_gaussians
